...
class server_sock(threading.Thread):
	...
	def run(self):	
		...
		while 1:
			...
			if not data: 
				break
			f = open("vm_image.png",'wb') #open as binary data
			...
			while (data):
				if(data.find(st)>=0):	
					...
					break;
				f.write(data)
                                print "receiving image from 10.0.0.1"
				data = s.recv(1024000)
		f.close()
...
class client_sock(threading.Thread):
	...
	def run(self):
		...
		port = 33300               # Reserve a port for your service.
		host1="10.0.0.2"
		s1.bind(('10.0.0.5',33000))
